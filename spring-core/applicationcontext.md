...
要 证明这个论述，我们从beanFactory.getBean(Class)方法就能得知结果。Spring容器就是一个巨大的bean工厂(BeanFactory)。Spring容器使用Ioc管理所有组成应用系统的组件。Spring容器会使用XML解析器(XmlBeanDefinitionReader)读取改属性值，并利用反射来创建该实现类的实例。